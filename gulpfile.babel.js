...
task('sass', () => {
	...
	return src([paths.src + '/styles/**/*.scss'
		...
		.pipe(devMode.production(gulpPostcss([cssnano])))  // делаем минификацию файлов // => style.css
		.pipe(devMode.production(rename({suffix: ".min"}))) // переименовываем файл // => min.style.css
		.pipe(devMode.development(sourcemaps.write('.'))) // => min.style.css
		.pipe(plumber.stop())
		.pipe(dest(paths.dist + '/css/'))  // dist/css/min.style.css
		.pipe(browserSync.stream()); // Триггерим Browsersync для обновления страницы
});
